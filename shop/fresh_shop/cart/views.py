...
def add_cart(request):
    if request.method == 'POST':
        # 加入购物车,需判断用户是否登录
        # 如果登录,加入到购物车中的数据,其实就是加入数据库中购物车表中(设计不好的办法)

        # 如果登录,加入数据库中的数据,存储在session中(设计相对较好)
        # 如果没有登录,则加入到购物车的数据,加入到session中
...
def cart(request):
    if request.method == 'GET':
        ...
        if session_goods:
            ...
            # 后台返回结构 [[goods object,number,total_price],...]
